...
def play_vid(path_vid): #Function for playong video
...
def get_fps(path_vid):   #Function for Frame Per Second(FPS)
    cap = cv2.VideoCapture(path_vid)
...
def vid_to_frame(path_vid, mode):   #Function to convert videos to frame
    vidcap = cv2.VideoCapture(path_vid)
    success, image = vidcap.read()
    count = 0
    while success:
        if mode == 'Fire':
            cv2.imwrite("Data/all/frame%d.jpg" % count, image)  # Save JPG file
        elif mode == 'Lake_Mary':
            cv2.imwrite("Data/lakemary/lake_frame%d.jpg" % count, image)  # Save JPG file
        elif mode == 'Test_Frame':
            # cv2.imwrite("frames/Test_frame/Fire/test_fire_frame%d.jpg" % count, image)  # Save JPG file for FIRE
            cv2.imwrite("Data/Test_frame/No_Fire/test_nofire_frame%d.jpg" % count, image)
...
def rename_all_files(path=None):   #Function to renaming the all file
