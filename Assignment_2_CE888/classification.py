...
def classify():
    ...
    test_ds = tf.keras.preprocessing.image_dataset_from_directory(
        "Data/Test", seed=1337, image_size=image_size, batch_size=batch_size, shuffle=True
    )
    ...
    test_fire_ds = tf.keras.preprocessing.image_dataset_from_directory(
        "Data/Test/Fire_test", seed=1337, image_size=image_size, batch_size=batch_size, shuffle=True)
    test_no_fire_ds = tf.keras.preprocessing.image_dataset_from_directory(
        "Data/Test/No_Fire_test", seed=1337, image_size=image_size, batch_size=batch_size, shuffle=True)
    fire_eval = model_fire.evaluate(test_fire_ds)
    no_fire_eval = model_fire.evaluate(test_no_fire_ds)
    true_fire = len(tf.io.gfile.listdir("Data/Test/Fire_test/Fire"))
    true_no_fire = len(tf.io.gfile.listdir("Data/Test/No_Fire_test/No_Fire"))
